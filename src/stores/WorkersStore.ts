...
import {type GameSaveSnapshot, nonNegativeIntegerSchema, nonNegativeNumberSchema, type WorkersSnapshot, workersSnapshotSchema} from "./shared"
...
export class WorkersStore {
    ...
    * loadWorkers() {
    ...
    calculateWorkerCost(worker: Worker, currentCount: number = this.hiredWorkers[worker.id] ?? 0): number {
        const baseCost = Math.ceil(worker.cost * Math.pow(worker.costMultiplier, currentCount))
